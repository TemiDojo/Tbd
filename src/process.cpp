...
namespace {
	void exit_with_perror(
			tbd::pipe& channel, std::string const& prefix) {
...
std::unique_ptr<tbd::process> tbd::process::launch(std::filesystem::path path, bool debug) {
...
std::unique_ptr<tbd::process> tbd::process::attach(pid_t pid) {
	if (pid == 0) {
		//Error: Invalid PID
...
void tbd::process::resume() {
	if (ptrace(PTRACE_CONT, pid_, nullptr, nullptr) < 0) {
		error::send_errno("Could not result");
	}
	state_ = process_state::running;
}

tbd::stop_reason::stop_reason(int wait_status) {
...
tbd::stop_reason tbd::process::wait_on_signal() {
	int wait_status;
	int options = 0;
...
void tbd::process::read_all_registers() {
...
void tbd::process::write_user_area(std::size_t offset, std::uint64_t data) {
...
void tbd::process::write_fprs(const user_fpregs_struct& fprs) {
	if (ptrace(PTRACE_SETFPREGS, pid_, nullptr, &fprs) < 0) {
		error::send_errno("Could not write floating point registers");
	}
}

void tbd::process::write_gprs(const user_regs_struct& gprs) {
