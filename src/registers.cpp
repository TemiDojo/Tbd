#include <libtbd/registers.hpp>
#include <libtbd/process.hpp>
#include <libtbd/bit.hpp>
#include <type_traits>
#include <algorithm>
#include <iostream>
...
namespace {
	template <class T>
		tbd::byte128 widen(const tbd::register_info& info, T t) {
			using namespace tbd;
...
tbd::registers::value tbd::registers::read(const register_info& info) const {
	...
	if (info.format == register_format::uint) {
		switch (info.size) {
			...
			default: tbd::error::send("Unexpected register size");
		}
	}
...
void tbd::registers::write(const register_info& info, value val) {
	auto bytes = as_bytes(data_);
	std::visit([&](auto& v) {
			if (sizeof(v) <= info.size) {
				...
				std::cerr << "tbd::register::write called with "
